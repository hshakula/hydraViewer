...
HdRprRenderTask::HdRprRenderTask(HdSceneDelegate* delegate, SdfPath const& id)
    : HdTask(id) {

}

HdRprRenderTask::~HdRprRenderTask() {

}

bool HdRprRenderTask::IsConverged() const {
    return m_pass ? m_pass->IsConverged() : true;
}

void HdRprRenderTask::Sync(HdSceneDelegate* delegate,
                           HdTaskContext* ctx,
    ...
    if ((*dirtyBits) & HdChangeTracker::DirtyParams) {
        HdRprRenderTaskParams params;

        auto value = delegate->Get(GetId(), HdTokens->params);
        if (TF_VERIFY(value.IsHolding<HdRprRenderTaskParams>())) {
            params = value.UncheckedGet<HdRprRenderTaskParams>();
        }
...
void HdRprRenderTask::Prepare(HdTaskContext* ctx,
...
void HdRprRenderTask::Execute(HdTaskContext* ctx) {
...
TfTokenVector const& HdRprRenderTask::GetRenderTags() const {
    return m_renderTags;
}
